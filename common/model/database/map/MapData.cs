...
public partial class MapData {
  ...
  public JsonObject ToJson() {
    ...
    foreach (var kvp in tileDict) {
      tilesJson[kvp.Key.ToOffsetCoords().ToString()] = kvp.Value.ToJson();
    }
  ...
  private static HexCoords ParseCoords(string coordsString) {
    // coordsString is in format "[x, y]"
    var trimmed = coordsString.Trim('[', ']');
    var parts = trimmed.Split(',');
    if (parts.Length != 2) {
      throw new FormatException($"Invalid coordinates format: {coordsString}");
    }
  ...
  public static MapData FromJson(JsonDocument json) {
    var element = json.RootElement;

    var mapId = element.GetProperty("id").GetString()!;
    var mapName = element.GetProperty("name").GetString()!;
    var mapDescription = element.GetProperty("description").GetString()!;
    var tilesElement = element.GetProperty("tiles");

    var tiles = new List<MapTileData>();
    foreach (var tileKvp in tilesElement.EnumerateObject()) {
      var coords = ParseCoords(tileKvp.Name);
      var tileData = MapTileData.FromJson(coords, tileKvp.Value);
      tiles.Add(tileData);
